...
class UFOTradingEngine:
    ...
    def should_open_new_trades(self, current_positions=None, portfolio_status=None, ufo_data=None, economic_events=None):
        """
        Determines if new trades can be opened based on UFO methodology with intelligent diversification
        Considers session timing, portfolio status, existing positions, diversification needs, and economic events
        """
        ...
        # Check session end conditions (now with economic events)
        should_close, close_reason = self.should_close_for_session_end(economic_events)
        if should_close:
